...
SD_Error SD_PowerON(void)
{
  ...
  /*初始化时的时钟不能大于400KHz*/ 
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;//SDIO初始化时钟分频系数
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;//在主时钟上升沿产生SDIO时钟
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;  //时钟旁路不使能 不使用bypass模式，直接用HCLK进行分频得到SDIO_CK
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;	//省电模式，总线空闲时SDIO时钟输出关闭
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;	 				//传输1位数据
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;//不使能数据流控制
  SDIO_Init(&SDIO_InitStructure);
  ...
  /*!< CMD Response: R7 */
  SDIO_CmdInitStructure.SDIO_Argument = SD_CHECK_PATTERN;//检测卡类型命令参数
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SEND_IF_COND;//识别卡命令cmd8
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;//短响应r7
  ...
  if (errorstatus == SD_OK)	//有响应则SD卡遵循SD协议2.0版本
  {
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0; //先把它定义成sdsc类型的卡
